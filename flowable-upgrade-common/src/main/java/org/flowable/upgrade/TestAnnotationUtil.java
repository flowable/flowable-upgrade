...
public class TestAnnotationUtil {
    ...
    public static boolean validateRunOnlyWithTestDataFromVersionAnnotation(Class<?> clazz) {
        ...
        RunOnlyWithTestDataFromVersion runOnlyWithTestDataFromVersion = clazz
                .getAnnotation(RunOnlyWithTestDataFromVersion.class);
        if (runOnlyWithTestDataFromVersion != null) {
            ...
            while (!versionMatches && index < versions.length) {
                EngineVersion requiredVersion = new EngineVersion(versions[index]);
                EngineVersion oldVersion = getOldVersion();
            ...
            if (versionMatches) {
                ...
                logger.info("@RunOnlyWithTestDataFromVersion found on test " + clazz
                        + ": version DOES NOT match. This test will not run.");
            }

            // A failed assumption does not mean the code is broken, but that
            // the test provides no useful information
    ...
    public static EngineVersion getOldVersion() {
        String oldVersion = System.getProperty("oldVersion");
        return new EngineVersion(oldVersion);
    }
