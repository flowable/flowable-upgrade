...
public class DataGenerator {
    ...
    private static void createSuspendTestData(ProcessEngine processEngine) {
        ...
        repositoryService.createDeployment()
                .addClasspathResource("org/activiti/upgrade/test/Activiti5To6Test.testSuspendProcess.bpmn20.xml")
                .deploy();

        RuntimeService runtimeService = processEngine.getRuntimeService();
        ProcessInstance processInstance = runtimeService.startProcessInstanceByKey("suspendProcess-activiti5", "suspended-process-instance");
    ...
    private static void createJobsTestData(ProcessEngine processEngine) {
        processEngine.getRepositoryService().createDeployment()
                .addClasspathResource("org/activiti/upgrade/test/Activiti5To6Test.processWithPlentyJobs.bpmn20.xml")
                .deploy();
        ...
        LOGGER.info("Case 1");
        ProcessInstance processInstance = runtimeService.startProcessInstanceByKey("plenty-of-jobs", "case01");
        ...
        LOGGER.info("Case 2");
        processInstance = runtimeService.startProcessInstanceByKey("plenty-of-jobs", "case02");
        ...
        processInstance = runtimeService.startProcessInstanceByKey("plenty-of-jobs", "case02bis");
        completeAsyncServiceTasks(processEngine, processInstance, "A");

        // Case 3: service task B/C/D/E/F are completed. There should be a timer throw active now.
        LOGGER.info("Case 3");
        processInstance = runtimeService.startProcessInstanceByKey("plenty-of-jobs", "case03");
        ...
        // Case 3bis: service task B/C/D/E/F are completed, but the parallel
        // gateways not yet
        LOGGER.info("Case 3bis");
        processInstance = runtimeService.startProcessInstanceByKey("plenty-of-jobs", "case03bis");
        completeAsyncServiceTasks(processEngine, processInstance, "A", "B", "C", "D", "E", "F");

        // Case 4: the intermediate timer is completed: user task G should be active
        LOGGER.info("Case 4");
        processInstance = runtimeService.startProcessInstanceByKey("plenty-of-jobs", "case04");
        completeAsyncServiceTasks(processEngine, processInstance, "A", "B", "C", "D", "E", "F");
        compleAsyncJoiningParallelGateways(processEngine, processInstance);
        compleAsyncJoiningParallelGateways(processEngine, processInstance);
        Job intermediateTimerJob = findJobWithType(processEngine, processInstance,
        ...
        LOGGER.info("Case 5");
        processInstance = runtimeService.startProcessInstanceByKey("plenty-of-jobs", "case05");
        completeAsyncServiceTasks(processEngine, processInstance, "A");
        ...
        LOGGER.info("Case 6");
        processInstance = runtimeService.startProcessInstanceByKey("plenty-of-jobs", "case06");
        completeAsyncServiceTasks(processEngine, processInstance, "A");
        ...
        LOGGER.info("Case 7");
        processInstance = runtimeService.startProcessInstanceByKey("plenty-of-jobs", "case07");
        completeAsyncServiceTasks(processEngine, processInstance, "A");
        boundaryTimer = findJobWithType(processEngine, processInstance, TimerExecuteNestedActivityJobHandler.TYPE);
        managementService.executeJob(boundaryTimer.getId());
        ...
        LOGGER.info("Case 8");
        processInstance = runtimeService.startProcessInstanceByKey("plenty-of-jobs", "case08");
        ...
        LOGGER.info("Case 9");
        processInstance = runtimeService.startProcessInstanceByKey("plenty-of-jobs", "case09");
        completeAsyncServiceTasks(processEngine, processInstance, "A");
        boundaryTimer = findJobWithType(processEngine, processInstance, TimerExecuteNestedActivityJobHandler.TYPE);
        managementService.executeJob(boundaryTimer.getId());
        completeAsyncServiceTasks(processEngine, processInstance, "After subprocess", "G", "H", "I", "J");
        ...
        LOGGER.info("Case 10");
        processInstance = runtimeService.startProcessInstanceByKey("plenty-of-jobs", "case10");
        completeAsyncServiceTasks(processEngine, processInstance, "A");
        runtimeService.suspendProcessInstanceById(processInstance.getId());
    }

    private static void createFailingServiceTaskData(ProcessEngine processEngine) {
        processEngine.getRepositoryService().createDeployment().addClasspathResource(
                "org/activiti/upgrade/test/Activiti5To6Test.processWithFailingServiceTask.bpmn20.xml").deploy();

        RuntimeService runtimeService = processEngine.getRuntimeService();
        ManagementService managementService = processEngine.getManagementService();

        ProcessInstance processInstance = runtimeService.startProcessInstanceByKey("activiti5-failing-job", "failed-job");
    ...
    private static void completeAsyncServiceTasks(ProcessEngine processEngine, ProcessInstance processInstance, String activityName) {
        ...
        // Find the job
        List<Job> jobs = managementService.createJobQuery().messages().processInstanceId(processInstance.getId())
                .list();
    ...
    private static void completeAsyncServiceTasks(ProcessEngine processEngine, ProcessInstance processInstance,
